...
//
// Like funcs.go, these are methods on *PacExecutor so build() can
// register them with runtime.Set directly.

// dnsResolveEx resolves host to all of its IPs (v4 and v6), semicolon
// separated, or "" if resolution fails or times out.
func (p *PacExecutor) dnsResolveEx(host string) string {
...
// v6) within p.dnsTimeout.
func (p *PacExecutor) isResolvableEx(host string) bool {
	ips, err := p.lookupHost(host)
...
func (p *PacExecutor) isInNetEx(ipAddress, ipPrefix string) bool {
...
// the Ex spec.
func (p *PacExecutor) myIpAddressEx() string {
