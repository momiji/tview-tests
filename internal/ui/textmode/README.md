...
- `Run(ctx context.Context, switchSignal <-chan struct{}) (Signal, error)` —
  puts stdin into raw mode and waits for a keypress, `switchSignal`, or
  `ctx` cancellation:
  - space, or a closed `switchSignal` → returns `SwitchToUI`
  - `q`, `Q`, Ctrl-C, or a cancelled `ctx` → returns `Quit`

  Pass `nil` for `switchSignal` to only react to keypresses (and `ctx`).

`switchSignal` lets some other part of the program request the switch
asynchronously (e.g. an automatic startup timer); `Run` treats it exactly
like a spacebar press and has no notion of why or when it fires. `ctx` lets
the proxy's shutdown (signal, auto-exit timeout, or self-update restart)
unblock `Run` so its terminal restore always runs — see
[../README.md](../README.md).
...
- **`run_unix.go`** (`!windows`, i.e. Linux and macOS) — the poll-based
  ...
  - **Poll loop, not a blocking `Read`.** `Run` polls stdin with
    ...
    `switchSignal` and `ctx` regularly, and means that whichever one fires,
    there is never a goroutine left blocked on stdin afterwards — which
    would otherwise race a later call to `Run` reading the same fd.
...
- **`run_windows.go`** — Windows has no equivalent of POSIX `poll()` on
  ...
  channel; every call to `Run` selects on that channel, `switchSignal`, and
  `ctx`. Sharing one persistent reader avoids ever having two goroutines
  racing a blocked `Read` for the same keypress, which a per-call reader
  (started and abandoned on every `Run` call) would risk on a platform
  where the read can't be cancelled.
...
- **Windows reader is best-effort.** The shared-reader approach above keeps
  the build green and never races stdin, but it has not been validated as
  thoroughly as the unix path; a more robust Windows implementation is
